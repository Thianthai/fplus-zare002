...
ใช้กฎกลางใน `~/.Codex/AGENTS.md` ทุกข้อ **แต่เปลี่ยน prefix จาก `Y*` เป็น `Z*` ทั้งหมด**
...
- **FE inline edit (SAPUI5 ≥ 1.136) ใช้กับ RAP draft BO ได้** — พิสูจน์แล้ว 2026-09-16 ทั้งใน
  ...
  RAP ว่าครอบคลุมเคสนี้** — Codex เคยสรุปผิดจากประโยคนั้นแล้วเสียเวลาไปครึ่งวัน
...
| **เอกสาร** (`docs/`, `README.md`, `AGENTS.md`) | **Codex** commit **และ push เอง** ทันทีที่ commit — ไม่ต้องรอสั่ง |

- ⚠️ **"ผม push ขึ้น Git" ของผู้ใช้ = abapGit push จาก ADT เท่านั้น** ไม่ใช่เอกสาร
  (ตีความผิดมาแล้ว 2026-09-07 → 09-15: เอกสารค้าง 13 commit ไม่ได้ push เพราะ Codex รอผู้ใช้)
  **abapGit push จาก ADT ไม่ได้ push เอกสารไปด้วย** — สองฝั่งแยกกันเด็ดขาด
- **ลำดับ push**: Codex push เอกสาร → ผู้ใช้ abapGit push · ถ้า abapGit push ไปก่อน
  local จะ diverge ต้อง `git rebase origin/main` ก่อน push (ไม่ conflict เพราะคนละไฟล์)
- Codex **ห้ามสร้างไฟล์ ABAP ลง repo** (`src/**/*.abap`, `*.ddls.asddls`, `*.asbdef` ฯลฯ)
...
- `.abapgit.xml` และ `package.devc.xml` เป็นของที่ **SAP serialize เอง** — ห้าม Codex เขียนหรือแก้มือ
- Codex คอยเช็ค `git log` / `git status` ว่าผู้ใช้ push object อะไรขึ้นมาแล้วบ้าง
