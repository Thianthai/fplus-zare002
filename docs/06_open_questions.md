...
| OQ-37 | **กด Reject ใบที่ `R` แล้วได้ข้อความ `Enter a Reject Reason for the following Payment Document No.: X, X` แทน 002** — DB เป็น `R`/`S` จริง · ข้อความไม่ใช่ของ message class `ZARE002` → น่าจะเป็น validate ฝั่ง client ของ UI extension ทีม Fiori (เปิดช่อง reason ของแถวที่เลือก + ปุ่ม Reject เอง) ซึ่งเช็ค "ทุกแถวต้องมี reason" แล้วไม่เรียก backend · กติกาต่างจาก backend (≥1/payment · 002 จาก DB) · ยืนยันได้จาก Network ว่ามี POST rejectItem ไหม · แนวทาง: extension ไม่ควร validate business rule เอง / backend จะแก้ 001·002 ให้ออกครั้งเดียวต่อ payment | ผู้ใช้ + ทีม Fiori | 8.8 | **hold** 2026-09-21 "ยังไม่ต้องแก้ ตอนนี้พฤติกรรม front end ยังแปลกๆ" · ไป Submit ก่อน | ⏸️ |
| OQ-39 | 🔴 **บรรทัดลูกหนี้ของ JE ที่ post ผ่าน API ได้ posting key `11` (credit memo) ไม่ใช่ `15` (incoming payment)** — พิสูจน์ 2026-09-22 เทียบ `3500000004` กับตัวอย่าง `3500000001` · เป็นข้อจำกัดของ `I_JournalEntryTP` ที่ POC `poc-payment` เจอมาก่อนแล้ว สั่งให้เป็น 15 ไม่ได้ · POC ยืนยันว่า **clear ได้เหมือนกัน** แต่ PK มีผลกับรายงานและ flag `IsUsedInPaymentTransaction` · tax code ที่บรรทัดนี้ได้ `**` ตัวอย่างว่าง | ผู้ใช้ + business | 8B | **ต้องถาม business ว่ารับได้ไหม** ถ้าไม่ได้ต้องเปลี่ยน API หรือให้ BOT post แทน | ⬜ |
| OQ-41 | **status `E` (Error) ไม่มีใครเขียนเลย** — spec เดิม R7 บอกว่า Submit ไม่สำเร็จให้ stamp `E` แต่ design ที่ตกลงภายหลัง: post JE ไม่สำเร็จ -> เก็บแค่ `submit_message` · BOT clear ไม่สำเร็จ -> เก็บแค่ `clearing_message` · status ยังเป็น `N` ทั้งคู่ · **ผล: ใบที่มีปัญหาขึ้นเป็น New บนจอ** icon แดงขึ้นเฉพาะ `R` · ต้องตัดสิน: (ก) คงไว้ ดูจากคอลัมน์ message (ข) stamp `E` เมื่อ post ไม่สำเร็จ (ค) stamp `E` เมื่อ BOT clear ไม่สำเร็จด้วย · ถ้า stamp `E` ต้องเช็คผลกระทบ: Submit รับ `N` และ `E` อยู่แล้วตาม spec · API #4 คิวดูจากเลขเอกสารไม่ดู status ไม่กระทบ · `salesforce_status = W` ก็ไม่มีใครเขียนเช่นกัน (ไม่เสียหาย) | ผู้ใช้ + ฟังก์ชันนอล | 8B | ไม่กระทบการทำงาน กระทบแค่การมองเห็นบนจอ | ⬜ |
...
| OQ-40 | **assignment ของบรรทัด G/L** — เราเคยใส่ posting date ทุกบรรทัด ตัวอย่างปล่อยว่างให้ sort key ของแต่ละบัญชีเติมเอง | **ปิด 2026-09-22** — ผู้ใช้เลือก **ยึดเอกสารตัวอย่างเป็นหลัก ไม่ส่ง assignment เลย** · สเปก `Submit Logic.docx` ไม่ได้ระบุเรื่องนี้ (ไม่มีคำว่า assignment/reference/item text ทั้งไฟล์) · sort key เป็น config ที่บัญชีตั้งใจไว้แล้ว ส่งทับจะทำให้รายงานที่เรียงตาม assignment เพี้ยน | 2026-09-22 |
| OQ-38 | **`SubmitMessage` แสดงบนจอที่ไหน** | **ปิด 2026-09-25** — เป็นคอลัมน์ `Submit Message` (position 130) คู่กับ `Clearing Message` (150) · `importance: #LOW` ให้ FE ซ่อนเองบนจอแคบ | 2026-09-25 |
