...
| OQ-34 | **token cache ของ Communication Arrangement ค้าง (Salesforce ไม่ส่ง `expires_in`)** | **ปิด 2026-09-21** — `ZCL_UTILITY=>get_sfdc_token` ขอใหม่ทุก call ผ่าน `ZCA_SFDC_TOKEN` (Basic · secret ใน Communication System) + `ZCL_ZARE002_SFDC_RESULT` ใส่ Bearer เอง · Reject จริงหลังแก้: composite ถึง SFDC HTTP 200 ไม่มี 401 อีก (ตกที่ข้อมูล NOT_FOUND ซึ่งเป็นคนละเรื่อง) · เหลือ 8C.10 ลบ `ZCS_REJECT_RESULT` ชุดเก่า · **ZARI002 ต้องย้ายตาม** | 2026-09-21 |
