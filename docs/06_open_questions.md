...
| OQ-30 | **API name ของ 5 field บน `cgcloud__Order_Payment__c` ตัวจริง** — spec ตาราง / JSON example / ตัวอย่าง Order object ใช้ naming ไม่ตรงกัน (`BST_SAP_ResponseDate__c` vs `BST_SAP_Response_Date__c`) · ขอให้ SFDC dev copy จาก Setup → Object Manager | SFDC dev | Phase 8A | **ตกลง 2026-09-20: ใช้ชื่อตาม JSON example ไปก่อน** เป็น constant 5 ตัวใน class · SFDC confirm แล้วค่อยแก้ | 🟨 |
