...
| OQ-18 | **BDEF บน tenant เป็น `with unmanaged save` โดยไม่มี saver** — **ส่ง code แก้แล้ว 2026-09-15** (ตัด `with unmanaged save` · คืน `persistent table`) รอ activate + push (commit `2e48c3e` เปลี่ยน · Phase 5 ลบ saver) → save `RejectReason` ไม่ลง table · **ต้องกลับเป็น `managed` + `persistent table`** ก่อนส่ง service ให้ทีม Fiori | ผู้ใช้ | Phase 6 | **บล็อกทีม Fiori** — service ที่ update แล้วไม่ persist ใช้ไม่ได้ | 🟨 |
