...
| OQ-26 | **Reject หลายใบพร้อมกัน — ใบที่ตกต้องลากใบที่ผ่านตกไปด้วยไหม** · พฤติกรรมตอนนี้ (ทดสอบ 2026-09-16): ติ๊ก A (ครบ) + B (ขาดเหตุผล) → Reject → **ไม่มีใบไหนถูก process** FE แจ้ง "No items were processed" + error ของ B — เพราะปุ่มตั้ง `#CHANGE_SET` = all-or-nothing · **ผู้ใช้บอกว่า requirement อาจต้องการแบบนี้** รอ confirm | ผู้ใช้ / business | Phase 7 | ไม่บล็อก — ถ้าต้องการให้ A ผ่านโดย B ตก มี 2 ทาง: (a) เปลี่ยนเป็น `#ISOLATED` — FE ยิงทีละแถว แต่ติ๊ก 2 item ใบเดียวกันจะยิงซ้ำ (รอบสองเจอ message 002) · (b) คง `#CHANGE_SET` แต่ใบที่ตกใส่แค่ `reported` error ไม่ใส่ `failed` → LUW commit ใบที่ผ่าน ผู้ใช้ยังเห็น error ของใบที่ตก — แนะนำ (b) ถ้าเลือกทางนี้ | ⬜ |
| OQ-27 | **ZARI002 ลบ `sap_payment_method` ออกจาก `ZTAR_I002_PYMT`** (2026-09-17 — functional ยืนยัน `PaymentMethod` จาก SFDC เป็นข้อมูลแสดงเฉย ๆ ไม่ใช้ post FI) → ZARE002 ต้องถอด field ออกจาก `ZI_ZARE002_PYMT` + `ZC_ZARE002` **ก่อน** ไม่งั้น object inactive · BDEF / draft table / handler ไม่กระทบ · Phase 8 post FI ใช้ `payment_method` (คำดิบ) ตั้งแต่ต้น | ผู้ใช้ | Phase 8 | **บล็อก ZARI002** จนกว่าเราจะ activate | 🟨 code ส่งแล้ว |
