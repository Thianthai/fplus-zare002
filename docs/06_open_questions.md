...
| OQ-17 | **ทีม Fiori จะทำให้คอลัมน์ Reject Reason แก้ได้ด้วยวิธีไหน** — (a) เปิด inline/mass edit มาตรฐานของ FE ใน manifest → **ต้องมี draft** · (b) เขียน controller เอง PATCH ตรง → **non-draft ง่ายกว่า** (1 call แทน Edit→PATCH→Activate) · ตอนนี้ tenant มี draft อยู่ **คงไว้ก่อน**จนกว่าจะรู้ | ผู้ใช้ + ทีม Fiori | Phase 6 | **บล็อกการตัดสินใจถอด draft** · ไม่บล็อก Phase 6 | ⬜ |
| OQ-18 | **BDEF บน tenant เป็น `with unmanaged save` โดยไม่มี saver** (commit `2e48c3e` เปลี่ยน · Phase 5 ลบ saver) → save `RejectReason` ไม่ลง table · **ต้องกลับเป็น `managed` + `persistent table`** ก่อนส่ง service ให้ทีม Fiori | ผู้ใช้ | Phase 6 | **บล็อกทีม Fiori** — service ที่ update แล้วไม่ persist ใช้ไม่ได้ | ⬜ |
