...
| 8.6 | Behavior pool | CLAS | `lhc_Item->rejectItem` validate ครบทุกใบ → composite → success เท่านั้นจึง buffer · `error_index` → แถวต้นเหตุ · `lsc_Item` เพิ่ม `salesforce_status = S` | 🟨 |
